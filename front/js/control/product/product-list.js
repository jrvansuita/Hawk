...
function showMessageTotals(info) {
  ...
  $('#totals').text(
    info
      ? window.Num.points(info.sum_quantity) + ' items e ' + window.Num.points(info.count) + ' skus'
...
function createImgProduct(product, index) {
  ...
  var img = $('<img>').attr('src', product.image).addClass('thumb').attr('onerror', "this.src='/img/product-placeholder.png'")
  ...
  var label = $('<label>').addClass('check-label').attr('for', checkBoxId).append(imgHolder)

  return $('<div>')
    .append(checked, label)
    .click(
      {
        index: index,
        sku: product.sku
...
function bindRangeSlider(data) {
  new RangeSlider($('.cost-filter'))
    .setRange(data.smaller_cost, data.greater_cost)
  ...
  new RangeSlider($('.price-filter'))
    .setRange(data.smaller_sell, data.greater_sell)
