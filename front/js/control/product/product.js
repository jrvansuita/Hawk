...
function requestProdutosFixes(callback) {
  _get('/diagnostics/fixes', { sku: product.codigo, grouped: true }, all => {
    window.fixes = all;
    callback(this);
  });
}
...
function loadObsHistory(child) {
  ...
  child.obs.split('\n').forEach(line => {
    if (line.includes('|')) {
      ...
      var $tr = $('<tr>').append(buildTextCol(date), buildTextCol(user), buildImgCol(platf.includes('Mobile') ? '/img/smartphone.png' : '/img/pc.png', platf), buildTextCol(type || 'Localização'), buildTextCol(data));
...
function loadStockHistory(childSku) {
  ...
  getStockRowsGrouped(childSku, grouped => {
    loadLayoutHistory(grouped);
...
function groupStockRows(rows) {
  ...
  return Object.values(groupArr).sort((a, b) => {
...
function loadLayoutHistory(rows) {
  rows.forEach(i => {
    ...
    var $tr = $('<tr>').append(buildTextCol(Dat.format(new Date(i.data))), buildTextCol(user), buildImgCol(isMobile ? '/img/smartphone.png' : '/img/pc.png', isMobile ? 'Mobile' : 'Desktop'), buildTextCol(parseInt(i.quantidade)).addClass('stock-val'), buildTextCol(obs));
...
function loadSumsStocks() {
  ...
  totals.forEach(item => {
    var total = $('#stock-history ' + item + ' .stock-val .child-value')
      .map(function () {
