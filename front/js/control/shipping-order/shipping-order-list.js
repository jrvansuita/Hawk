  ...
  dots.each((index, each) => {
    Dropdown.on(each)
      ...
      .item(window.transportList.icons.default.icon, 'Coletado', (helper) => {
        _post('/shipping-order/collected', { id: helper.data.id }, () => {
          var s = $(each.closest('tr')).find('.ship-status');
          s.text('Coletado').addClass('collected');
          each.closest('tr').removeClass('open-item').addClass('collected-item');
        });
      });
  });
