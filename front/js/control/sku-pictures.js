      ...
      $previewImg
        .append(
          $('<img>')
            ...
            .attr('src', '/product/image-redirect?sku=' + sku)
        )
        .click(() => {
...
function savePictureToDatabase() {
  _post(
    '/pictures/from-insta',
    {
      instaPost: getInstaId(),
      skus: getSelectedSkus().join(','),
    },
    data => {
...
function createSingleTag(sku) {
  return $('<span>').addClass('tag copiable').append(sku).attr('data-value', sku.toString().toLowerCase());
}
