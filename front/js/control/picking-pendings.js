$(document).ready(() => {
  $('.checkable-avatar').change(function () {
    ...
    $(this)
      .closest('.pending-box')
      .find('.pending-count')
      .text(selecteds == 0 ? count : selecteds + '/' + count);
  });
...
function getFirstBottomBarOptions(pending) {
  ...
  var span = $('<span>')
    .addClass('info-value small-font last-update')
    .append('Última alteração: ' + (pending.updateDate ? Dat.formatwTime(new Date(pending.updateDate)) : ''));
  div.append(span);
...
function buildProductFirstCol(item, slim, pending) {
  ...
  sku
    .addClass('info-value sku ' + (isSwapableSku ? 'sku-input' : 'copiable'))
    .val(item.codigo)
    .text(item.codigo)
  ...
  if (!slim) {
    ...
    var descHolder = $('<label>')
      .addClass('info-value desc no-wrap')
      .text(Util.ellipsis(Util.getProductName(item.descricao, true), 25));
...
function createVoucherPendingMenuOption(drop, pending) {
  if (
    !pending.sale.items.some(each => {
      return each.changed;
    }) &&
    isTrueStr(pending.sendEmail)
  ) {
