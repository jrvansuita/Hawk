...
module.exports = class History extends DataAccess {
  ...
  static error (e, title, addtoMessage, userId) {
    var _userId = userId || (e.userId ? e.userId : 0)

    console.error(e)
    if (process.env.IS_PRODUCTION) {
      new History(2, _userId, title || 'Erro de Sistema', addtoMessage ? addtoMessage + '\n' : '' + Err.xprss(e), 'Falha').upsert()
    }
  }
  ...
  static job (title, message, tag, userId) {
    new History(4, userId || 0, title, message, tag).upsert()
  }
}
