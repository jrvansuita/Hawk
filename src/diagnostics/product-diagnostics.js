...
module.exports = class ProductDiagnostics {
  ...
  _analyzeProducts(product, stocks, callback) {
    ...
    if (isGenderMissing(attrBundle, product)) {
      this._storeFix(product, 'GENDER');
    }

    if (isSeasonMissing(attrBundle.names, product)) {
      this._storeFix(product, 'SEASON');
    }

    if (isCollectionYearMissing(attrBundle.names, product)) {
      this._storeFix(product, 'COLLECTION_YEAR');
    }
...
function isBrandMissing(product, attrNames) {
  var isMissingAttrs = !attrNames.includes('Marca') || !attrNames.includes('Fabricante');
  var isMissingFeed = product.feedProduct && (!product.feedProduct.brand || !product.feedProduct.manufacturer);
  return isMissingAttrs || isMissingFeed;
}
...
function isGenderMissing(attrBundle, product) {
  ...
  var isMissing = index == -1 || !['Masculino', 'Feminino', 'Unissex', 'Unisex'].includes(val);
  return isMissing || (product.feedProduct && !product.feedProduct.gender);
}

function isSeasonMissing(attrNames, product) {
  return !attrNames.includes('Estacao') || (product.feedProduct && !product.feedProduct.season);
}

function isCollectionYearMissing(attrNames, product) {
  return !attrNames.includes('Coleção') || (product.feedProduct && !product.feedProduct.year);
}
