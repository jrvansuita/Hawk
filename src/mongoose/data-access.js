module.exports = class DataAccess {
  ...
  static aggregate(query, callback) {
    this.staticAccess().aggregate(query, function (err, res) {
      if (callback) {
        callback(err, res);
      }
    });
  ...
  static upsert(query, data, callback) {
    delete data.__v;
    this.staticAccess().findOneAndUpdate(
      query,
      data,
      {
        upsert: true,
  ...
  upsert(callback) {
    ...
    this.classAccess().findOneAndUpdate(
      this.getPKQuery(),
      this,
      {
        upsert: true,
