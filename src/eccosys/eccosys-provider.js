var EccosysApi = require('./eccosys-api.js');
...
module.exports = class EccosysProvider extends EccosysApi {
  ...
  salesBySituation(situation) {
    return this.param('$orderBy', 'data,ASC')
      .get('pedidos/situacao/' + situation)
      .multiple();
  }
  ...
  product(skuOrEan) {
    return this.get('produtos/' + (Num.isEan(skuOrEan) ? 'gtin=' + skuOrEan : skuOrEan))
      .single()
      .prepare(data => {
  ...
  skus(skus) {
    return this.get('produtos/' + (Array.isArray(skus) ? skus.join(';') : skus))
      .multiple()
      .prepare(data => {
        Util.bindProductAttrs(data);
        return data;
  ...
  manufacturers() {
    return this.get('fornecedores').multiple();
  }

  brands() {
    return this.get('marcas').multiple();
  }
};
