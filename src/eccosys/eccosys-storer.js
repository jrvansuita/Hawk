const EccosysApi = require('./eccosys-api.js');
...
module.exports = class EccosysStorer extends EccosysApi {
  product(sku) {
    return {
      ...
      attrs: () => {
        return {
          put: (body) => {
            return this.post('produtos/' + sku + '/atributos')
              .setBody(body)
              .single();
          },
        };
  ...
  stock(sku, body) {
    return this.post('estoques/' + sku)
      .setBody(body)
      .single();
  }

  nfe(user, saleNumber) {
    return this.post('nfes/' + saleNumber)
      .setBody({})
      .withUser(user)
      .single();
  }
  ...
  retryNfe(user, idNfe) {
    return this.post('nfes/' + idNfe + '/autorizar')
      .setBody({})
      .withUser(user)
      .single();
  }
  ...
  cancelNfe(user, numberNfe, body) {
    return this.post('nfes/' + numberNfe + '/cancelar')
      .nfeParams()
      .setBody(body)
      .withUser(user)
      .single();
  ...
  shippingOrder(user) {
    ...
    return {
      ...
      update: (id, idsNfeArr) => {
        return this.put('ordem-de-coleta/' + id)
          .setBody(idsNfeArr)
          .withUser(user);
      },
